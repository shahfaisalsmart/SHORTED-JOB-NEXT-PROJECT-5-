...
// Finally checking those process which is not availble till yet. Means this is newly created process.
            for(i=0;i<number;i++)
        	{
        		if(proc[i].status==0) // it is in NEW status.
        		{
        			if(timer >= proc[i].AT )  // time is greater or equal to arrival time 
        			{
                        // then only we can change it's status to the in Ready state.
        				proc[i].status=1;
...
// finally total time calculation.
    int total_wt=0; // total time variable.
    for(i=0;i<number;i++)
    {
        total_wt=total_wt+proc[i].WT; // adding every process waiting time.
    }
// for total arrival time 
    int total_at=0;  // total arrival time variable
    for(i=0;i<number;i++)
    {
        total_at=total_at+proc[i].AT;  // adding in the loop for every process
    }
 ...
 //   printf("status :--   COMPLETE  == CM \n");
   ...
   // int z=number-1;
    ...
    printf("\n\n\n");
    // AND FINALLY ALL ARE COMPETED. PROJECT COMPLETED.
    return 0;
}
